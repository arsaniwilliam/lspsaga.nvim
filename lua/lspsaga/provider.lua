...
function Finder:auto_open_preview()
	...
	if next(content) ~= nil then
		...
		if max_width > 42 then
			...
			-- Don't show preview window if too short
...
local function preview(timeout_ms, lsp_method)
	local preview_method
	local preview_title
	if lsp_method == "def" then
		preview_method = "textDocument/definition"
		preview_title = "Definition"
	elseif lsp_method == "type-def" then
		preview_method = "textDocument/typeDefinition"
		preview_title = "Type"
	end
	...
	if vim.tbl_islist(result) then
		for _, definition in pairs(result) do
			if not vim.tbl_isempty(definition) and not vim.tbl_isempty(definition.result) then
				...
				content = vim.list_extend({
					config.definition_preview_icon .. preview_title .. " Preview: " .. short_name,
					"",
				}, content)
		...
		require("notify").notify("[saga] result is not list!!")
	end
end

function lspfinder.preview_type_definition(timeout_ms)
	preview(timeout_ms, "type-def") --"textDocument/typeDefinition")
end

function lspfinder.preview_definition(timeout_ms)
	preview(timeout_ms, "def") -- "textDocument/definition")
end
